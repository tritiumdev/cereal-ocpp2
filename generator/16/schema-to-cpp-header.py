...
for input_file in args.input:
    ...
    scheme = ocpp16_schema.Scheme()
    
    master_name = schema["title"]
    # 1.6 ext related..
    ...
    if schema["type"] == "object":
        scheme.objects.append(ocpp16_schema.create("Object", name=master_name, attributes=schema, master=master_name))
    ...
    while len(candidates):
        ...
        if attributes["type"] == "string":
            if "enum" in attributes.keys():
                if name[-8:] == "EnumType":
                    scheme.enums.append(ocpp16_schema.create("Enum", name=ocpp16_schema.digit_prefix(name), attributes=attributes))
                elif name == "status":
            ...
            scheme.objects.append(ocpp16_schema.create("Object", name=name, attributes=attributes, master=None))
            ...
            if attributes["items"]["type"] == "object":
                these_attributes = attributes["items"]
                this_name = name + "Entry"
                scheme.objects.append(ocpp16_schema.create("Object", name=this_name, attributes=these_attributes, master=None)) 
                candidates.extend([[name,attributes] for name,attributes in these_attributes["properties"].items()])
            # hmmm...
            pass
...
common_file = ocpp16_schema.HeaderFile(args.common_header, args.namespace, args.include)
...
for input_file,scheme in sorted(schemes.items()):
    ...
    output_file = ocpp16_schema.HeaderFile(output_file, args.namespace, args.include)
